...
1.  Memastikan *User Experience* (UX) yang sederhana dan logis.
2.  Mencegah *bottleneck* atau *break* dalam alur kerja antar-modul (misalnya, dari UGD ke Rawat Inap, atau dari RME ke Kasir).
...
| **5. Pembayaran** | Kasir mencari pasien di daftar *Billing Pending* $\rightarrow$ Memproses rincian biaya. | Billing Engine menarik semua biaya terkait `Visit`. | Kasir |
...
| Langkah | Aksi Pengguna (User) | Sistem Memproses/Merender | Modul |
| :--- | :--- | :--- | :--- |
| **1. Registrasi RI** | Admin/UGD mendaftarkan pasien untuk RI (atau *transfer* dari UGD/RJ) $\rightarrow$ Memilih jenis jaminan/pembayaran. | Membuat entitas `Visit` dengan `visit_type='RAWAT_INAP'`. | Pendaftaran |
...
| **3. Disposisi (Keputusan)** | Dokter UGD menentukan: A. **Pulang**, B. **Rawat Jalan Lanjut**, atau C. **Rawat Inap**. | **A:** Mengubah status `Visit` ke 'Ready for Billing'. **B/C:** Memulai proses *handover* ke modul RJ/RI. | UGD |
