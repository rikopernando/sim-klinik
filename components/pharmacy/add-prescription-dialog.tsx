  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (axios.isAxiosError(err)) {
        setError(
          err.response?.data?.error || err.response?.data?.message || "Gagal menambahkan resep"
        )
      } else {
      ...
      <DialogContent className="max-h-[90vh] max-w-2xl overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            ...
            {doctor && (
              <span className="text-foreground mt-1 block font-medium">Dokter: {doctor.name}</span>
            )}
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            >
              <SelectTrigger>
                <SelectValue placeholder={isLoadingDrugs ? "Memuat obat..." : "Pilih obat"} />
              </SelectTrigger>
              <SelectContent>
                {drugs.map((drug) => (
          ...
          <div className="space-y-2">
            ...
            <Select
              ...
              onValueChange={(value) => setFormData({ ...formData, addedByPharmacistId: value })}
              disabled={isLoadingPharmacists}
            >
              <SelectTrigger>
                <SelectValue
                  placeholder={isLoadingPharmacists ? "Memuat farmasi..." : "Pilih petugas farmasi"}
                />
              </SelectTrigger>
              <SelectContent>
