...
async function seedBillingTestData() {
    ...
    try {
        ...
        if (poliGeneral.length === 0) {
            const inserted = await db.insert(polis).values({
                name: "Poli Umum",
        ...
        const existingPatient = await db.select().from(patients).where(eq(patients.mrNumber, "MR999001")).limit(1);
        ...
        const existingVisit = await db.select().from(visits).where(eq(visits.visitNumber, visitNumber)).limit(1);
        ...
        const existingMedicalRecord = await db.select().from(medicalRecords).where(eq(medicalRecords.visitId, testVisit.id)).limit(1);
