...
async function migrateUsername() {
  ...
  try {
    ...
    await db.execute(
      sql`UPDATE "user" SET "username" = SPLIT_PART("email", '@', 1) WHERE "username" IS NULL`
    )
    ...
    try {
      await db.execute(
        sql`ALTER TABLE "user" ADD CONSTRAINT "user_username_unique" UNIQUE("username")`
      )
    } catch (error: unknown) {
      // Constraint already exists, ignore error
      const err = error as { code?: string }
      if (err.code !== "42P07") {
        // 42P07 = duplicate object error
        throw error
      }
