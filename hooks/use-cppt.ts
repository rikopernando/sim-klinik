...
export function useCPPT(): UseCPPTReturn {
  ...
  const createEntry = useCallback(async (data: CPPTInput) => {
    ...
    try {
      ...
      const errorMessage = err instanceof Error ? err.message : "Failed to create CPPT entry"
  ...
  const fetchEntries = useCallback(async (visitId: number): Promise<CPPT[]> => {
    ...
    try {
      ...
      const errorMessage = err instanceof Error ? err.message : "Failed to fetch CPPT entries"
      setError(errorMessage)
