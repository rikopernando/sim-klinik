    ...
    async (request: NextRequest) => {
        try {
            ...
            for (const item of prescriptions) {
                if (!item.prescriptionId || !item.inventoryId || !item.dispensedQuantity || !item.fulfilledBy) {
                    return NextResponse.json(
                        { error: "Each prescription must have prescriptionId, inventoryId, dispensedQuantity, and fulfilledBy" },
                        { status: 400 }
                    );
                }
            }
