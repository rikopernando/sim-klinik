...
export async function GET(request: NextRequest) {
  try {
    ...
    const results = await db
      ...
      .from(icd10Codes)
      .where(
        or(ilike(icd10Codes.code, searchPattern), ilike(icd10Codes.description, searchPattern))
      )
    ...
    return NextResponse.json({ error: "Failed to search ICD-10 codes" }, { status: 500 })
  }
}
