...
export async function POST(request: NextRequest) {
  try {
    ...
    const room = await db.select().from(rooms).where(eq(rooms.id, validatedData.roomId)).limit(1)
    ...
    if (room[0].availableBeds <= 0) {
      return NextResponse.json({ error: "No available beds in this room" }, { status: 400 })
    }
...
export async function PATCH(request: NextRequest) {
  try {
    ...
    // Get room info
    const room = await db.select().from(rooms).where(eq(rooms.id, assignment[0].roomId)).limit(1)
    ...
    return NextResponse.json({ error: "Failed to discharge from bed" }, { status: 500 })
  }
}
