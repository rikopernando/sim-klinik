...
class SSEManager {
    ...
    addConnection(channel: string, controller: ReadableStreamDefaultController) {
        ...
        console.log(`[SSE] New connection to channel: ${channel}. Total connections: ${this.connections.get(channel)!.size}`);
    }
    ...
    removeConnection(channel: string, controller: ReadableStreamDefaultController) {
        ...
        if (channelConnections) {
            ...
            console.log(`[SSE] Connection removed from channel: ${channel}. Remaining: ${channelConnections.size}`);
    ...
    broadcast(channel: string, payload: NotificationPayload) {
        ...
        console.log(`[SSE] Broadcasting to ${channelConnections.size} connections in channel: ${channel}`);
...
export function createSSEStream(channel: string) {
    return new ReadableStream({
        start(controller) {
            ...
            const encoder = new TextEncoder();
            const connectionMessage = encoder.encode(`data: ${JSON.stringify({
